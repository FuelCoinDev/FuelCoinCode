...
uint256 hashGenesisBlock("0x00000545daa8f19c12ae1856b551293f03c89447aadbeb3a41a4403c287fc0b4");
...
bool CTxOut::IsDust() const
{
    ...
    // so dust is a txout less than 54 uFUEL
...
static int64 nTargetTimespan = 3 * 60 * 60; // three hours
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
	...
	// Use old parameters before fork height
	if (pindexLast->nHeight+1 < FORK_ONE_HEIGHT)
	{
		nTargetTimespan = 4 * 60 * 60; // four hours
		nTargetSpacing = 0.5 * 60; // 30 seconds
	...
	{
	    if (nActualTimespan < nTargetTimespan/50)
        nActualTimespan = nTargetTimespan/50;
		if (nActualTimespan > nTargetTimespan*50)
        nActualTimespan = nTargetTimespan*50;
	} else {
...
void ThreadScriptCheck() {
    RenameThread("FuelCoin-scriptch");
...
void SetGenesisHash() {
        ...
        // Genesis block
        const char* pszTimestamp = "FuelCoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 0x950364d1 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        assert(block.hashMerkleRoot == uint256("0x8befdee80601a0a8ebdbc68fbabb4a2e8341691b9e40dac8963dcbfd0836a761"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf5, 0xd3, 0xa3, 0xd0 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FuelCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FuelCoinMiner : ProcessBlock, block not accepted");
    }
...
void static BitcoinMiner(CWallet *pwallet)
{
    ...
    {
        printf("FuelCoinMiner terminated\n");
        throw;
    }
}
